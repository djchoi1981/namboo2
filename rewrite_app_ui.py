with open('/Users/djchoi81/Desktop/church-schedule-app/app.js', 'w') as f:
    ...
    document.getElementById('save-menu-names-btn').addEventListener('click', () => {
        ...
        localStorage.setItem('churchMenuNames', JSON.stringify(menuNames));
        applyMenuNames();
        showToast("메뉴명이 저장되었습니다.");
        
        // Update current page title if active
        if(navCommittee.classList.contains('active')) pageTitle.textContent = `${menuNames.committee} 등록 및 현황`;
        if(navCell.classList.contains('active')) pageTitle.textContent = `${menuNames.cell} 및 현황`;
        if(navVehicle.classList.contains('active')) pageTitle.textContent = `${menuNames.vehicle} 및 현황`;
    });
    ...
    navCommittee.addEventListener('click', (e) => {
        ...
        pageTitle.textContent = `${menuNames.committee} 등록 및 현황`;
    ...
    navCell.addEventListener('click', (e) => {
        ...
        registrationSection.style.display = 'block';
        pageTitle.textContent = `${menuNames.cell} 등록 및 현황`;
        monthNavigation.style.display = 'none';
        switchFormType('cell');
        renderCellList();
    });
    ...
    navVehicle.addEventListener('click', (e) => {
        ...
        registrationSection.style.display = 'block';
        pageTitle.textContent = `${menuNames.vehicle} 등록 및 현황`;
        monthNavigation.style.display = 'none';
