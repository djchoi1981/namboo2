with open('/Users/djchoi81/Desktop/church-schedule-app/app.js', 'w') as f:
    ...
    function switchFormType(type) {
        ...
        if (type === 'vehicle') {
            ...
            formTitle.innerHTML = '<i class="ph ph-calendar-plus"></i> 차량 배차 등록';
        } else if (type === 'cell') {
            locationLabel.innerHTML = '예약 장소 <span class="required" style="color: var(--danger);">*</span>';
            formTitle.innerHTML = '<i class="ph ph-calendar-plus"></i> 셀/모임 장소 등록';
        } else {
    ...
    function renderAdminEventsList() {
        ...
        document.querySelectorAll('.admin-delete-btn').forEach(btn => {
            btn.addEventListener('click', (ev) => {
                const id = ev.target.getAttribute('data-id');
                if(confirm("정말로 이 예약을 강제 삭제하시겠습니까? (관리자 권한)")) {
