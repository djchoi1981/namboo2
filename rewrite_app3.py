with open('/Users/djchoi81/Desktop/church-schedule-app/app.js', 'w') as f:
    ...
    function switchFormType(type) {
        ...
        document.querySelectorAll(`#fields-${type} input, #fields-${type} select`).forEach(el => el.setAttribute('required', 'true'));
        
        if (type === 'vehicle') {
            locationLabel.innerHTML = '목적지 <span class="required" style="color: var(--danger);">*</span>';
            formTitle.innerHTML = '<i class="ph ph-calendar-plus"></i> 차량 운행 예약';
        } else if (type === 'cell') {
            locationLabel.innerHTML = '예약 장소 <span class="required" style="color: var(--danger);">*</span>';
            formTitle.innerHTML = '<i class="ph ph-calendar-plus"></i> 셀/모임 장소 예약';
        } else {
            locationLabel.innerHTML = '장소 <span class="required" style="color: var(--danger);">*</span>';
            formTitle.innerHTML = '<i class="ph ph-calendar-plus"></i> 위원회/부서 일정 등록';
        }
    }
    ...
    navCommittee.addEventListener('click', (e) => {
        ...
        pageTitle.textContent = '위원회/부서 일정 등록 및 현황';
    ...
    navCell.addEventListener('click', (e) => {
        ...
        registrationSection.style.display = 'block';
        pageTitle.textContent = '셀/모임 장소 예약 및 현황';
        monthNavigation.style.display = 'none';
        switchFormType('cell');
        renderCellList();
    });
    ...
    navVehicle.addEventListener('click', (e) => {
        ...
        registrationSection.style.display = 'block';
        pageTitle.textContent = '차량 배차 예약 및 현황';
        monthNavigation.style.display = 'none';
    ...
    btnEditEvent.addEventListener('click', () => {
        ...
        // Navigate to the correct tab so the form is visible
    ...
    function checkForConflicts(excludeId = null) {
        ...
        const conflictingEvents = events.filter(e => {
            ...
            if (type === 'vehicle') {
                ...
                if (e.eventType === 'vehicle') return false; 
                if (e.location !== targetResource) return false; 
            }
        ...
        filteredEvents.forEach(e => {
            ...
            item.innerHTML = `
                ...
                <div class="dept-item-meta">
                    ...
                    <span style="margin-left: auto;">[셀/모임] ${e.cellName} (${e.reserver})</span>
                </div>
            `;
        ...
        filteredEvents.forEach(e => {
            ...
            item.innerHTML = `
                ...
                <div class="dept-item-meta">
                    <span><i class="ph ph-calendar"></i> ${e.date}</span>
                    <span><i class="ph ph-clock"></i> ${e.startTime} ~ ${e.endTime}</span>
                    <span style="color: #f59e0b; font-weight: 500;"><i class="ph ph-map-pin"></i> 목적지: ${e.location}</span>
                    <span style="margin-left: auto;">[차량예약] ${e.vehicle} (${e.reserver})</span>
                </div>
            `;
